...
class GmObject():
    ...
    def parse_dict(self, dict):
        ...
        for key in dict:
            setattr(self, key, myunicode(dict[key]))
...
class Search(Songlist):
    ...
    def load_songs(self, start=0, number=20):
        if start < len(self.songs):
            return self.songs[start:start + number]
        else:
...
class DirSearch(Directory):
    ...
    def load_songlists(self, start=0, number=20):
        if start < len(self.songlists):
            return self.songlists[start:start + number]
        else:
